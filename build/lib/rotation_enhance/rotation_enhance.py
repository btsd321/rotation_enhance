                ...
                for angle in self.angles_list:
                    ...
                    try: 
                        if len(yolo_targets) != len(rotated_targets):
                            raise ValueError(f"yolo_targets and rotated_targets length not match, yolo_targets: {len(yolo_targets)}, rotated_targets: {len(rotated_targets)}")
                        for target in rotated_targets:
                            find_match = False
                            for yolo_target in yolo_targets:
                                if target.is_matched(yolo_target):
                                    find_match = True
                                    target.box_w = yolo_target.box_w
                                    target.box_h = yolo_target.box_h
                                    break
                            if not find_match:
                                raise ValueError(f"未找到匹配的目标: {target}, yolo_targets: {yolo_targets}")
                    except Exception as e:
                        raise ValueError(f"Error matching targets: {e}")
